 		...
 		for (prelayer3.x = prelayer3.prearea.area.x+prelayer3.break_x_value;prelayer3.x <= prelayer3.prearea.area.xMax;prelayer3.x += prelayer3.prearea.step.x)
 		{
			...
			if (button_export)
			{
				...
				if (heightmap_output)
				{
					...
					export_bytes[((heightmap_x*2)+(heightmap_y*preterrain.heightmap_resolution*2))] = byte_hi;
					export_bytes[(((heightmap_x*2)+1)+(heightmap_y*preterrain.heightmap_resolution*2))] = byte_lo;
				}
	...
	for (var count_terrain: int = 0;count_terrain < terrains.Count;++count_terrain) {
		...
		if (active) {
			...
			if (!terrains[count_terrain].rtp_script) {
				terrains[count_terrain].rtp_script = UnityEngineInternal.APIUpdaterRuntimeServices.AddComponent(terrains[count_terrain].terrain.gameObject, "Assets/TerrainComposer/Scripts/terraincomposer_save.js(7440,70)", "ReliefTerrain");
...
function assign_rtp_single(terrain1: terrain_class)
{
	...
	if (!terrain1.rtp_script) {
		terrain1.rtp_script = UnityEngineInternal.APIUpdaterRuntimeServices.AddComponent(terrain1.terrain.gameObject, "Assets/TerrainComposer/Scripts/terraincomposer_save.js(7466,39)", "ReliefTerrain");
	}
}
